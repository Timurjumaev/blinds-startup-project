...
def get_stage_users(search: str = None, id: int = 0, page: int = 0, limit: int = 25, stage_id: int = None, db: Session = Depends(database),
    ...
    return all_stage_users(search, page, limit, stage_id, db, current_user)
...
def create_stage_user(new_stage_user: CreateStage_user, db: Session = Depends(database),
                      current_user: CreateUser = Depends(get_current_active_user)):
    role_verification(current_user, inspect.currentframe().f_code.co_name)
    create_stage_users_s(new_stage_user, db, current_user)
...
def update_stage_user(this_stage_user: UpdateStage_user, db: Session = Depends(database),
                      current_user: CreateUser = Depends(get_current_active_user)):
    role_verification(current_user, inspect.currentframe().f_code.co_name)
    update_stage_users_s(this_stage_user, db, current_user)
    raise HTTPException(status_code=200, detail="Amaliyot muvaffaqiyatli amalga oshirildi")
...
def delete_stage_user(id: int, db: Session = Depends(database),
                      current_user: CreateUser = Depends(get_current_active_user)):
    role_verification(current_user, inspect.currentframe().f_code.co_name)
    delete_stage_user_r(id, db, current_user)
    raise HTTPException(status_code=200, detail="Amaliyot muvaffaqiyatli amalga oshirildi")
